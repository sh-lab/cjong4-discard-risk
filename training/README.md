...
現在の初期学習には段階教師 `staged-v1` を使います。実ロン牌だけFF、全員安全は00、
残りはリーチ・副露・残りツモ枚数に応じた16〜192です。
通常局面も対局単位で分割し、`--ron-fraction 0 --selection-metric mse` で学習します。
`per_target`で段階の再現、`ff_*`で実ロンFFへの出力を確認します。
学習v2では飽和した出力へも近似勾配を通し、全00基準・危険側誤差・AUCを確認できます。
以前のv1チェックポイントは書き出し可能ですが、途中再開せず新しいrunで学習します。

## 旧00/FF教師を生成する（比較・回帰検証用）

この節のデータは現在の段階教師と混ぜません。初期学習には上記standardの手順を使います。
